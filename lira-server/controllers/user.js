...
function signUp(req, res) {
  ...
  if (!password || !repeatPassword) {
    res.status(404).send({ message: 'Las constraseñas con obligatiorias' })
  } else {
    if (password !== repeatPassword) {
      res
        .status(404)
        .send({ message: 'Las contraseñas tienen que ser iguales iguales' })
    } else {
      //Vamos a encryptar la contraseña
...
function deleteUser(req, res) {
  ...
  User.findByIdAndRemove(id, (err, userDeleted) => {
    if (err) {
      res.status(500).send({ message: 'Error del servirdor' })
    } else {
      if (!userDeleted) {
        res.status(404).send({
        ...
        user.save((err, userStored) => {
          if (err) {
            ...
            if (!userStored) {
              ...
              res.status(200).send({ user: 'Usuario creado correctamente' })
            }
          }
        })
      }
